...
Markov_states = [pandas.read_csv(
    "./{}/Markov_states_{}.csv".format(model,i), index_col=0).values
    for i in range(120)
]
transition_matrix = [pandas.read_csv(
    "./{}/transition_matrix_{}.csv".format(model,i), index_col=0).values
    for i in range(120)
]
...
def generator(random_state,size):
    ...
    for t in range(T-1):
        noise = numpy.exp(
            random_state.multivariate_normal(mean=[0]*4, cov=sigma[t%12],size=size))
        inflow[:,t+1,:] = noise * (
...
if model == 'SA100':
    ...
    fig.tight_layout()
    fig.savefig("./result/{}.png".format(model), dpi=600)
